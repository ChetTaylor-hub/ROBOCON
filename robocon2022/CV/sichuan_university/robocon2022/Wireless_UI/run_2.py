...
class CamShow(QMainWindow, Ui_Camshow):
    ...
    def show_camera(self):
        ...
        self.update()
        # self.frame.setPixmap(QtGui.QPixmap.fromImage(showImage))
        # self.camerLayout.addWidget()
        # self.pww.show(showImage)
        # self.pw.image(show)
        # pp.drawImage(showImage)

    def paintEvent(self, event):
        pp = QPainter(self)
        pp.drawImage(self.rect(), self.showImage)
    ...
    def clickButton_cont(self):
        ...
        self.start1_camera()
        self.mode = 0
        # self.timer_camera.timeout.connect(self.paintEvent)
...
if __name__ == '__main__':
    mpu_ser = Comcontrol(port='COM1',  # 单片机串口实例化/dev/ttyTHS0     COM1
                             ...
                             timeout=0.8,
                             parity='N')
    # _thread.start_new_thread(mpu_ser.read_data, ())
