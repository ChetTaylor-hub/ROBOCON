...
def DrawFitCircles(circles, cimg):
    circles = np.uint16(circles)  # 转换类型为整型
...
def StdShowImage(image, WindowName):  # 图像窗口可以放缩的
        ...
        if (angle is not None) and not (right_bound <= angle <= left_bound) and (dis is not 0):
            ...
            if i > 400:
                ...
                if circles is not None:
                    Mpu.write("~".encode() + str(quantity).encode() + "e".encode())  # 向单片机串口发送圆数量，帧头~
                    for circle in circles:
                        for j in circle:
                            Mpu.write("#".encode() + str(j[0]).encode() + "e".encode())  # 向单片机串口发送x，帧头#
                            Mpu.write("%".encode() + str(j[1]).encode() + "e".encode())  # 向单片机串口发送y，帧头%
                            Mpu.write("^".encode() + str(j[2]).encode() + "e".encode())  # 向单片机串口发送r，帧头^
                    # # ----调试用（绘制拟合圆并展示）-----
