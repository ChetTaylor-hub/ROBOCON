...
if __name__ == '__main__':
    ...
    while cap.isOpened():
        ...
        #print(bias_red_y, bias_blue_y)
        end = time.time()
        #print('FPS=%d'%(int(1/(end-start))))
        print(frame.shape)
        cv.rectangle(frame, (int(width/2 - check_width/2), int(height/2 - check_height/2)), (int(width/2 + check_width/2), int(height/2 + check_height/2)), 2)  # 划矩形线把图像包起来
        #cv.rectangle(src, (int(width/2 - check_width/2), int(height/2 - check_height/2)), (int(width/2 + check_width/2), int(height/2 + check_height/2)), (0, 255, 0), 2)  # 划矩形线把图像包起来
