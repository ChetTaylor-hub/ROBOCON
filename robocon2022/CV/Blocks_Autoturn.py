...
'''灯光色域'''
#广角120
red_lower = np.array([137, 33, 183])  # HSV阈值低
red_upper = np.array([255, 255, 255])  # HSV阈值高
blue_lower = np.array([44, 188, 189])  # HSV阈值低
blue_upper = np.array([255, 255, 255])  # HSV阈值高
white_lower = np.array([95, 66, 183])  # HSV阈值低
white_upper = np.array([251, 90, 255])  # HSV阈值高
'''
#广角170
red_lower = np.array([126, 84, 175])  # HSV阈值低
red_upper = np.array([255, 255, 255])  # HSV阈值高
blue_lower = np.array([31, 218, 98])  # HSV阈值低
blue_upper = np.array([255, 255, 255])  # HSV阈值高
white_lower = np.array([86, 34, 210])  # HSV阈值低
white_upper = np.array([255, 167, 255])  # HSV阈值高
'''
...
def detect_circle(frame, lower, upper):
    ...
    hsv = cv.cvtColor(frame, cv.COLOR_BGR2HSV)  # BGR格式转换成HSV颜色格式
    mask1 = cv.inRange(hsv, lower, upper)  # 得到mask，入口参数（图像，HSV颜色低值，HSV颜色高值）
    mask1 = cv.erode(mask1, None, iterations=4)
    mask1 = cv.dilate(mask1, None, iterations=4)
